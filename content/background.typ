...
Review comment systems provide a structured way to discuss and track issues in evolving artifacts. Instead of treating findings as ephemeral output, a review system models issues as threads anchored to a location in a document (e.g., a file and line range) with one or more comments that capture rationale and discussion. This representation supports collaboration by allowing multiple instructors to contribute context, propose alternatives, and converge on a decision.

A key feature of review systems is state. Threads are typically marked as open or resolved, enabling reviewers to separate active work from completed decisions and to measure progress. For AI-assisted workflows, state is also needed to distinguish between issues that have been accepted and fixed, those that have been intentionally dismissed, and those that require re-evaluation because the exercise has changed. The concept of an \"outdated\" thread is particularly useful when anchors are tied to line content: if the surrounding content changes, the system can warn that the original context may no longer apply and prompt reviewers to revisit the thread.

For the scope of this thesis, review comments provide the foundation for turning consistency checking into an actionable workflow. They enable persistent storage of detected issues, integrate manual instructor feedback and AI-generated findings into a unified interface, and provide an auditable record of what was changed and why. This is essential for iterative exercise development, where artifacts evolve over time and where the cost of repeatedly re-interpreting raw tool output would otherwise remain high.
