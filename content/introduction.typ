...
The system also provides mechanisms for applying or discarding suggestions directly from the editor. When an instructor accepts a suggestion, Artemis replaces the existing code with the suggestion and automatically creates a new exercise version. The implementation includes validation and conflict checks to ensure that suggestions remain consistent with the current state of the file, even if it has been modified since the check was generated. This approach reduces repetitive manual editing, accelerates the review process, and allows instructors to focus on conceptual validation while the system handles mechanical consistency corrections.
