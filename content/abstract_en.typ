...
This thesis extends Artemis with AI-assisted review tools that make consistency checks accessible and actionable. The thesis introduces a persistent review comment system and stores each consistency issue as a review comment in that system. The exercise editor visualizes issue comments inline with severity, affected locations, and suggested fix descriptions, and the editor provides an overview for fast navigation. Instructors and editors can preview and apply suggested code changes directly in the editor while retaining human control over final changes. These contributions improve the reliability of programming exercises and reduce manual overhead.

Methodologically, the work follows a design-oriented software engineering approach. It derives requirements from instructor workflows, maps them into a system design with persistent issue storage and editor integration, and implements the system within Artemis.
